...
- **Auto-mute non-instrument pages when not in front** (standard adopted 2026-05-20). Any chriso.org page that runs ambient audio *but isn't itself an instrument the user is playing* — landings like `/Sandbox/`, `/TestCases/`, the entrance variations — should suspend its audio and halt processing (early-return grain/crackle schedulers; ramp master to 0; `audioCtx.suspend()`) on `document.visibilitychange` to hidden, and resume on visible. Real instruments (FX Processor, Buffer Shuffler, DronorParty, luminous-strings, etc.) keep playing — the user is performing with them. Implementation pattern: a `pageVisible` flag plus a `visibilitychange` handler that ramps `master.gain` to 0 over ~150 ms then calls `audioCtx.suspend()`; reverse on resume. The same flag guards spawn loops so they no-op while hidden.
